...
for (i, imgPath) in enumerate(imgPaths):
    ...
    feature2 = ft.local_binary_pattern(img, 32, 4, method='ror').flatten()
...
#model1 =KNeighborsClassifier(p=1, n_neighbors=92, weights='uniform', algorithm='auto') #达到0.56
...
for row2 in range(rows):
    ...
    if a1>a2:
        ...
        if a2>a3:
            ...
            max_num = c[row2]
    # 3.如果第一个概率矩阵中三个值中最大值超过0.5，就直接选取
    if full_pred[0][row2,a[row2]]>0.5:
